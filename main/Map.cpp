...
string const Map::MENU_EXIT[] = {"Exit Dungeon", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " "};
string const Map::MENU_INVENTORY[] = {
     "+-------------+"
    ,"| INVENTORY   |"
    ,"+-------------+"
    ,"| Gold        | "
...
bool Map::move(char direction)
{
    ...
    {
    case 'w': // if user inputs w, move ups if it is an allowed move
    ...
    for (int i = 0; i < num_rows_; i++)
    {
        ...
        if (i == 11) {
            ...
            if (isRoomLocation(player_position_[0], player_position_[1])) {
                ...
                if (MENU_EXIT[i] != " ") {
                    cout << "\t" << i + 1 << ": " << MENU_EXIT[i];
                } else {
                    cout << "\t\t";
                }
            } else {
                if (MENU_NORMAL[i] != " ") {
                    cout << "\t" << i + 1 << ": " << MENU_NORMAL[i];
                } else {
                    cout << "\t\t";
                }
            }
